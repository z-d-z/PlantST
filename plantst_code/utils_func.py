...
def _ensure_tissue_positions_list(visium_path: Union[str, Path]):
    ...
    csv_new = spatial_dir / "tissue_positions.csv"
    ...
    if csv_new.exists():
        ...
        # 有些数据列名一致，直接抽取；写成无header的list格式更兼容老scanpy
        df = df[required]
    ...
    if parquet.exists():
        ...
        # 有些parquet可能把barcode放在index里
        if "barcode" not in df.columns and df.index.name == "barcode":
            df = df.reset_index()
        elif "barcode" not in df.columns and df.index.dtype == "object":
            # 兜底：把index当barcode尝试
            df = df.reset_index().rename(columns={"index": "barcode"})

        missing = [c for c in required if c not in df.columns]
        if missing:
            raise ValueError(f"Found tissue_positions.parquet but missing columns: {missing}")

        df = df[required]

        # 强制数值列为 int（避免某些工具把它当字符串）
        for c in required[1:]:
    ...
    raise OSError(
        f"Could not find any tissue positions file in {spatial_dir} "
        f"(expected tissue_positions_list.csv / tissue_positions.csv / tissue_positions.parquet)"
    )

def read_10X_Visium(path, 
                    genome = None,
                    count_file ='filtered_feature_bc_matrix.h5',
                    # count_file ='normalized_expression.h5ad',
                    library_id = None, 
                    load_images =True, 
                    quality ='hires',# 图片的分辨率  hires高分辨率图像  fulres完全分辨率图像
                    image_path = None):
    _ensure_tissue_positions_list(path)  # <-- 新增这一行
    ...
    # adata = sc.read_h5ad(r"D:\Anaconda\anaconda\envs\macpst\MACPST-main\data\orchid_flowers_1/GSM5939574_processed.h5ad")
...
def refine(
    sample_id, #样本的标识符（ID）
    pred, #一个包含每个样本预测标签的列表或数组
    dis, #距离矩阵，dis 是一个二维矩阵，其中 dis[i][j] 表示样本 i 和样本 j 之间的距离
    shape="hexagon"#"hexagon"（六边形邻域，常用于Visium数据）或 "square"（方形邻域，常用于ST数据）
    ):
    refined_pred=[]#保存细化后的预测标签
